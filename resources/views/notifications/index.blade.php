        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between mb-10 gap-4">
                <div>
                    <div class="flex items-center gap-3 mb-2">
                        ...
                        <h1 class="text-2xl sm:text-3xl font-bold text-slate-800">Notifikasi</h1>
                    </div>
                    <p class="text-slate-500 text-sm sm:text-base ml-[52px]">Informasi terbaru terkait aktivitas akun Anda.
                    </p>
                </div>
                ...
                @if ($unread > 0)
                    <form action="{{ route('notifications.read-all') }}" method="POST" class="ml-[52px] sm:ml-0">
                        @csrf
                        <button type="submit"
                            class="inline-flex items-center gap-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 px-5 py-2.5 rounded-xl text-sm font-semibold transition-colors border border-emerald-100">
            ...
            <div class="bg-white rounded-2xl shadow-sm shadow-black/5 border border-slate-100 overflow-hidden">
                ...
                @forelse($notifications as $notif)
                    <div
                        class="flex gap-4 sm:gap-5 px-5 sm:px-6 lg:px-8 py-5 border-b border-slate-100 last:border-0 transition-colors duration-150
                           {{ !$notif->is_read ? 'bg-emerald-50/40 hover:bg-emerald-50/60' : 'hover:bg-slate-50/50' }}">

                        {{-- ICON TYPE --}}
                        <div class="flex-shrink-0 mt-0.5">
                            <div
                                class="w-11 h-11 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center
                            ...
                            @elseif(str_contains($notif->type, 'role') || str_contains($notif->type, 'updated'))
                            ...
                            @else 
                                ...
                                @elseif(str_contains($notif->type, 'role') || str_contains($notif->type, 'updated'))
                                    <svg class="w-5 h-5 sm:w-6 sm:h-6 text-violet-600" fill="none" stroke="currentColor"
                                        stroke-width="2" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                    </svg>
                                @else
                                    <svg class="w-5 h-5 sm:w-6 sm:h-6 text-slate-500" fill="none" stroke="currentColor"
                                        stroke-width="2" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-4">
                                <h3
                                    class="text-sm sm:text-base font-bold text-slate-800 {{ !$notif->is_read ? 'text-emerald-900' : '' }}">
                                    {{ $notif->title }}
                                </h3>
                                <div class="flex items-center gap-2 flex-shrink-0">
                                    ...
                                    @endif
                                    <span class="text-[11px] sm:text-xs text-slate-400 whitespace-nowrap font-medium">
                        ...
                        <div class="flex-shrink-0 flex flex-col items-end gap-2">
                            ...
                            {{-- WRAPPER ALPINE --}}
                            <div x-data="{ showDeleteModal: false }" class="flex-shrink-0 flex flex-col items-end gap-2">
                                ...
                                <div x-show="showDeleteModal" x-cloak x-transition:enter="transition ease-out duration-200"
                                    ...
                                    {{-- CONTENT POPUP --}}
                                    <div
                                        class="relative bg-white rounded-2xl shadow-2xl shadow-black/10 p-6 sm:p-8 w-full max-w-sm text-center z-10">

                                        {{-- ICON --}}
                                        <div
                                            class="w-16 h-16 mx-auto mb-5 rounded-2xl bg-red-100 flex items-center justify-center">
                                        ...
                                        <p class="text-sm text-slate-500 leading-relaxed mb-6">
                                            Notifikasi ini akan dihapus secara permanen dan tidak bisa dikembalikan.
                                        </p>

                                        {{-- FORM ACTION (TERSEMBUNYI SAMPAI DIKONFIRMASI) --}}
                                        <form action="{{ route('notifications.destroy', $notif->id) }}" method="POST"
                                            id="delete-form-{{ $notif->id }}">
                                            @csrf @method('DELETE')
                                        </form>
                                        ...
                                        <div class="flex gap-3">
                                            ...
                                            <button
                                                ...
                                                type="button"
                                                class="flex-1 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-sm font-semibold text-white shadow-sm shadow-red-500/20 transition-colors">
                                                Ya, Hapus
                                            </button>
                                        </div>
                                    </div>
                ...
                @empty
                    ...
                    <div class="py-24 text-center">
                        ...
                        <h3 class="text-slate-700 font-bold text-lg">Semua Bersih</h3>
                        <p class="text-slate-400 text-sm mt-2 max-w-sm mx-auto leading-relaxed">Belum ada notifikasi masuk.
                            Semua informasi penting akan muncul di sini.</p>
                    </div>
                @endforelse
